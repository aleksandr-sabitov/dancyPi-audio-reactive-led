...
<html lang="en">
    <head>
        <title>Sabitov Media Center</title>
        <meta charset="utf-8">
...
div[id^='a_']:hover { underline: none;}
    ...
    <body class="container-fluid">
        ...
        <div id="a_head" class="row text-center" onclick="js_refresh_page();" style="background: lightblue; color: white; padding-bottom: 15px;">
            <h2>Sabitov Media Server</h2>
        </div>
        ...
        <div id="all_buttons" class="row">
            <div class="col-xs-4 col-lg-4">
                <div class="text-center">
                    <button onclick="js_send_simple_command_with_stop('/html/toggle', 1500);" type="button" class="btn btn" style="color:white;background-color:crimson">On-Off<br>Radio</button>
                    <button onclick="js_send_simple_command('/html/st', 0);" type="button" class="btn btn" style="color:white;background-color:darkcyan">Stations<br>List</button>
            ...
            <div class="col-xs-4 col-lg-4">
                <div class="text-center">
                    <button id="vu" onclick="js_send_simple_command('/html/volumeup', 500);" type="button" class="btn btn" style="color:white;background-color:forestgreen">Volume<br>Up</button>
                    <button id="vd" onclick="js_send_simple_command('/html/volumedown', 500);" type="button" class="btn btn" style="color:white;background-color:forestgreen">Volume<br>Down</button>
            ...
            <div class="col-xs-4 col-lg-4">
                <div class="text-center">
                    <button onclick="js_send_simple_command('/html/onledscroll', 0);" type="button" class="btn btn" style="color:yellow;background-color:fuchsia">CRAZY<br>Led</button>
                    <button onclick="js_send_simple_command('/html/onledspectrum', 0);" type="button" class="btn btn" style="color:darkblue;background-color:lightskyblue">SOFT<br>Led</button>
...
/histprev        /hp        play previous station from history''',
    ...
    def _handle_client_connection(self, address, request):
        ...
        elif self._path in ('/onledscroll'):
            if self._is_html:
                ...
                pass  
        elif self._path in ('/onledspectrum'):
            if self._is_html:
                if self.led_music_subprocess_pid > 0:
                    os.killpg(os.getpgid(self.led_music_subprocess_pid), signal.SIGTERM)  
                    self.led_music_subprocess_pid = 0 
                ...
                pass 
        elif self._path in ('/onledenergy'):
            if self._is_html:
                if self.led_music_subprocess_pid > 0:
                    os.killpg(os.getpgid(self.led_music_subprocess_pid), signal.SIGTERM)  
                    self.led_music_subprocess_pid = 0 
                ...
                pass                                      
        elif self._path in ('/offled'):
            if self._is_html:
                # if self.pigpiod_pid > 0:
                #     print(self.pigpiod_pid)
                #     led_subprocess = subprocess.Popen(["sudo kill -9 " + self.pigpiod_pid], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT, shell=True, preexec_fn=os.setsid)  
                #     self.pigpiod_pid = 0 
                if self.led_music_subprocess_pid > 0:
                    os.killpg(os.getpgid(self.led_music_subprocess_pid), signal.SIGTERM)  
                    self.led_music_subprocess_pid = 0 
                ...
                pass      
        elif self._path in ('/darkredled'):
            if self._is_html:
                # if self.pigpiod_pid == 0:
                #     pigpiod_subprocess = subprocess.Popen(["sudo pigpiod"], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT, shell=True, preexec_fn=os.setsid)
                #     sleep(0.3)
                #     self.pigpiod_pid = pigpiod_subprocess.pid
                #     print(self.pigpiod_pid)
                if self.led_music_subprocess_pid > 0:
                    os.killpg(os.getpgid(self.led_music_subprocess_pid), signal.SIGTERM)  
                    self.led_music_subprocess_pid = 0 
